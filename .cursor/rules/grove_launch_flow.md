...
Read before editing `HarvestGate`, `VineAf`, `GroveNote`.

## AppsFlyer

- `wire()` in `GroveApp.onCreate` — `init` + listeners, **no** `start`, **no** network.
- `kick(activity)` only after `VineReach.hasRadio()` / `canTalk()`.
...
### pending
- No adapter → `QuietCanopy` on frame one. Nothing persisted.
...
`extractUrl` reads `jx4_note` **and** raw `url` / `link` / `target_url`.
Firebase-drawn notification blocks never run `GroveNoteService`.
