...
10. JS sentinels `__jx4sa` / `__jx4kb` — unique per project.
11. File chooser: `FileChooserParams.createIntent()`, no storage permission.
12. Back: `web.goBack()` step-by-step through history. First page is a no-op, never `finish()`.
13. Redirect loop: `retryPending` + cover stays up. Resume from `deepestHop`. Soft-cap hops at 16. Never show `ERR_TOO_MANY_REDIRECTS`. Do **not** `clearHistory()`.
...
15. Cleartext hops upgraded to https via `LatchUrl.clean`.
16. `CHANNEL_ID` = `ch_jx4grove` matches manifest meta-data.
...
23. Hide status + nav bars on Launch / Invite / Offline / WebView (`GroveFill`, after `setContentView`).
...
25. Skip cooldown is 3 **calendar** days from local midnight. Re-offer on WebView `onStart` after the date rolls, not only via `HarvestGate`.
26. R8: minify+shrink on release. `-repackageclasses jx`. Do **not** `-keep grove.**`. Keep only manifest entry classes. No blanket WebViewClient / @Composable keep.
