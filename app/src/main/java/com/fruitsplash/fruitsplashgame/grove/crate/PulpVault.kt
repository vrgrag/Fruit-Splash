...
class PulpVault(context: Context) {
    ...
    fun readMode(): GroveLane = GroveLane.fromWire(plain.getString(K_MODE, null))

    fun writeMode(mode: GroveLane) {
    ...
    fun armInviteCooldown() {
        ...
        val until = java.time.LocalDate.now(zone)
            .plusDays(GroveMark.INVITE_COOLDOWN_SECONDS / 86_400L)
            .atStartOfDay(zone)
            .toEpochSecond()
