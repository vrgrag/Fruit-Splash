 ...
 */
class NectarAf(private val ctx: Context) {
    ...
    fun wire() {
        ...
        val key = NectarBrand.attributionKey
        if (key.isEmpty()) {
    ...
    fun kick(host: Activity) {
        wire()
        if (NectarBrand.attributionKey.isEmpty()) return
    ...
    suspend fun collect(firstLaunch: Boolean): JSONObject = coroutineScope {
        val budget = when {
            retraced -> NectarBrand.ATTRIBUTION_TIMEOUT_MS_RETRACE
            firstLaunch -> NectarBrand.ATTRIBUTION_TIMEOUT_MS
            else -> NectarBrand.ATTRIBUTION_TIMEOUT_MS_RESUME
        }
        val linkWait = async {
            withTimeoutOrNull(NectarBrand.DEEP_LINK_TIMEOUT_MS) { deepDone.await() }
        }
        val dataWait = async {
