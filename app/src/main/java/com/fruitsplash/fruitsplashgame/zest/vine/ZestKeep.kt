...
internal class ZestKeep(context: Context) {
    ...
    fun readLane(): ZestLane = ZestLane.fromWire(plain.getString(K_LANE, null))

    fun writeLane(lane: ZestLane) {
        plain.edit().putString(K_LANE, lane.wire).apply()
    }
    ...
    fun armInviteCooldown() {
        plain.edit()
            .putLong(K_INVITE_UNTIL, nowSeconds() + ZestIds.INVITE_COOLDOWN_SECONDS)
            .apply()
    }
    ...
    private companion object {
        const val PLAIN_FILE = "orch_memo_bin"
        const val SECRET_FILE = "orch_sealed_bin"
        const val K_LANE = "route_mark"
        const val K_CACHED_LINK = "page_blob"
        const val K_LINK_TTL = "page_ttl"
        const val K_PUSH_LINK = "drop_blob"
        const val K_PUSH_TOKEN = "token_hex"
        const val K_INVITE_UNTIL = "ask_after"
        const val K_PUSH_ALLOWED = "note_ok"
        const val K_PUSH_BLOCKED_OS = "note_halt"
        const val K_PUSH_OS_ASKED = "note_asked"
    }
}
